...
class BSK_GFBLCV_Dashboard_FF_Settings {
	...
	function __construct() {
        ...
        if ( BSK_GFBLCV_Dashboard_Common::bsk_gfblcv_is_form_plugin_supported('FF') ) {
            add_filter( 'frm_add_form_settings_section', array( $this, 'bsk_gfblcv_ff_form_settings_fun' ), 10, 2 );
            add_filter( 'frm_form_options_before_update', array( $this, 'bsk_gfblcv_ff_form_settings_save_fun' ), 20, 2 );
        }
        
	}
	
	
    function bsk_gfblcv_ff_form_settings_fun( $sections, $values ) {
        
        $sections[] = array(
            'name'		=> 'BSK Blacklist',
            'anchor'	=> 'bsk_blacklist_ff_form_settings_tab',
            'function'	=> 'bsk_gfblcv_ff_form_settings_render_fun',
            'class'	    => $this
        );
        return $sections;
    }

    function bsk_gfblcv_ff_form_settings_render_fun( $values ) {
        ...
        //form settings
        $bsk_gfblcv_form_settings = maybe_unserialize( get_option( BSK_GFBLCV_Dashboard_Formidable_Forms::$_bsk_gfblcv_ff_form_settings_option_name_prefix . $values['id']) );
			...
			<form action="<?php esc_url( $action_url ); ?>" method="POST">
                ...
                <div class="bsk-gfblcv-form-settings-actions-container" style="display: <?php echo $form_settings_actions_container_display; ?>">
                    ...
                    <table class="gforms_form_settings" cellspacing="0" cellpadding="0">
                        ...
                        $form_actions = FrmFormAction::get_action_for_form( $form_id, 'all', $filters );
                        if( $form_actions && is_array($form_actions) && count($form_actions) > 0 ){
                            $html = '
                                <tr class="bsk-gfblcv-form-settings-option-tr bsk-gfblcv-notificaitons-to-skip" style="display: '.$form_settings_skip_notifications_container_display.'">
                                    <th>&nbsp;</th>
                                    <td>
                                        <p>Check Actions &amp; Notifications to skip:</p>
                                        ...
                                        <ul>';
                            foreach( $form_actions as $notification_id => $notification_setting ){
                                ...
                                $html .= '<li>
                                            <label>
                                            <input type="checkbox" name="bsk_gfblcv_notification_to_skip[]" value="'.$notification_id.'"'.$checked_str.' /> '.$notification_setting->post_title.'
                                            </label>
                                          </li>';
                        ...
                        $form_confirmations = array( 'YES' );
                        if( isset($form_confirmations) && is_array($form_confirmations) && count($form_confirmations) > 0 ){
                            $html = '
                                <tr class="bsk-gfblcv-form-settings-option-tr bsk-gfblcv-confirmations-to-go" style="display: '.$form_settings_confirmations_to_go_container_display.'">
                                    <th>&nbsp;</th>
                                    <td>
                                        <p>Change <strong>ON SUBMIT</strong><span class="frm_help frm_icon_font frm_tooltip_icon" title="" data-original-title="Formidable forms\' ON SUBMIT settings on General tab"></span> action to</p>';
                            
                            $html .= '<div class="bsk-gfblcv-tips-box">
                                            <p>This feature requires a <span style="font-weight: bold;">CREATOR</span>( or above ) license for Pro verison. </p>
                                            <p>To buy a license, please <a href="'.BSK_GFBLCV::$_plugin_home_url.'" target="_blank">click here >></a></p>
                                      </div>';
                            $message_selected = $redirect_selected = $show_page_selected = '';
                            if ( isset( $bsk_gfblcv_form_settings['confirmation_to_go'] ) ) {
                                if ( $bsk_gfblcv_form_settings['confirmation_to_go'] == 'message' ) {
                                    $message_selected = 'selected';
                                } else if ( $bsk_gfblcv_form_settings['confirmation_to_go'] == 'redirect' ) {
                                    $redirect_selected = 'selected';
                                } else if ( $bsk_gfblcv_form_settings['confirmation_to_go'] == 'page' ) {
                                    $show_page_selected = 'selected';
                                }
                            }
                            
                            $html .= '
                                        <select name="bsk_gfblcv_confirmation_to_go" id="bsk_gfblcv_confirmation_to_go_ID">
                                            <option value="message"'.$message_selected.'>Show Message</option>
                                            <option value="redirect"'.$redirect_selected.'>Redirect to URL</option>
                                            <option value="page"'.$show_page_selected.'>Show Page Content</option>
                                        </select>
                                    </td>
                                </tr>';
                            echo $html;
                ...
                <input type="hidden" name="save_bsk_gfblcv_settings" />
			</form>
		</div>         
        <?php
    }
    
    function bsk_gfblcv_ff_form_settings_save_fun( $options, $values ) {
            
        if ( isset( $values['save_bsk_gfblcv_settings'] ) ) {
            $bsk_gfblcv_form_settings = array();
            
            $bsk_gfblcv_form_settings['enable'] = sanitize_text_field($values['bsk_gfblcv_form_settings_enable']) == 'ENABLE' ? true : false; ;
            $action_when_hit = array( 'BLOCK' );
            $bsk_gfblcv_form_settings['actions'] = $action_when_hit;
            $bsk_gfblcv_form_settings['notification_to_skip'] = array();
            $bsk_gfblcv_form_settings['confirmation_to_go'] = '';
            $bsk_gfblcv_form_settings['save_blocked_data'] = sanitize_text_field($values['bsk_gfblcv_save_blocked_data']);
            $bsk_gfblcv_form_settings['notify_administrators'] = 'NO';
            $bsk_gfblcv_form_settings['notify_send_to'] = 'NO';
            $bsk_gfblcv_form_settings['delete_entry'] = 'NO';
            $bsk_gfblcv_form_settings['blacklist_message'] = wp_unslash(sanitize_text_field($values['bsk_gfblcv_blacklist_message']));
            $bsk_gfblcv_form_settings['whitelist_message'] = wp_unslash(sanitize_text_field($values['bsk_gfblcv_whitelist_message']));
            $bsk_gfblcv_form_settings['emaillist_message'] = wp_unslash(sanitize_text_field($values['bsk_gfblcv_emaillist_message']));
            $bsk_gfblcv_form_settings['iplist_message'] = wp_unslash(sanitize_text_field($values['bsk_gfblcv_iplist_message']));
            
            update_option( 
                            BSK_GFBLCV_Dashboard_Formidable_Forms::$_bsk_gfblcv_ff_form_settings_option_name_prefix . $values['id'], 
